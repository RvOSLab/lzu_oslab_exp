...
{
    // NOTE: 相关函数实现位置: 
    //         PLIC 相关: trap/plic.c 文件
    //         UART 相关: trap/uart 文件夹
    //         RTC  相关: trap/rtc 文件夹
    //       相关函数声明位置: 
    //         PLIC 相关: include/plic.h 文件
    //         UART 相关: include/uart.h 文件
    //         RTC  相关: include/rtc.h 文件

    // TODO: 1. 从 PLIC 中获取外中断号
    //       2. 处理 UART 中断(中断号为 UART_SUNXI_IRQ 或 UART_QEMU_IRQ)
    //          具体要求: 1. 处理 UART 中断
    //                   2. 完成实验内容 2.2
    //       3. 处理 RTC 中断(中断号为 RTC_GOLDFISH_IRQ 或 RTC_SUNXI_IRQ)
    //          具体要求: 1. 处理 RTC 中断
    //                   2. 设置下一次 RTC 中断时间为当前 RTC 时间 + 1 s(1000000000 ns)
    //                   3. 刷新游戏画面
    //       4. 对其他中断输出 "Unknown external interrupt: {中断号}\n."
    //       5. 调用 PLIC 中断完成操作
    return tf;
}
