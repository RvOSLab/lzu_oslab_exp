...
{
    volatile struct uart_qemu_regs *regs = (struct uart_qemu_regs *)uart_device.uart_start_addr;
    // TODO: 从 UART_RBR 读取数据并返回 
}

static void uart_16550a_directly_write(int8_t c)
{
    volatile struct uart_qemu_regs *regs = (struct uart_qemu_regs *)uart_device.uart_start_addr;
    // TODO: 将要被传送的数据写入至 UART_THR 寄存器
}

static void uart_16550a_putc(int8_t c)
{
    volatile struct uart_qemu_regs *regs = (struct uart_qemu_regs *)uart_device.uart_start_addr;
    // TODO: 检查 UART_LSR[THRE] 以检查发送 FIFO 是否为空。为空则调用 uart_16550a_directly_write(c) 发送数据。
}
...
static void uart_16550a_interrupt_handler()
{
    ...
    // TODO:
    // 读取 UART_LSR[DR] 检查 RX_FIFO 状态
    // 若该位为 1，继续从 UART_RBR 读取字符 c 并使用 game_keyboard_update(c) 更新游戏状态
}
