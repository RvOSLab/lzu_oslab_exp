...
struct bucket_desc* take_empty_bucket(uint8_t alloc_size) {
    ...
    // TODO: [lab4] 
    //       - 分配新物理页作为新桶并使用 init_bucket_page() 函数将其初始化
    //       - 完成 if 分支, 针对是否为 SPECIAL_BUCKET_SIZE_LOG2 大小实现以下功能:
    //          - 使用当前桶第一块作为桶描述符 或 分配新的桶描述符(bucket)
    //          - 初始化桶描述符(.refcnt, .freeidx)
    //       - 初始化桶描述符(.page)
    //       - 注意：代码编写过程中注意物理地址与虚拟地址的转换
    //       -      开启分页后所有访问的地址都是虚拟地址，
    //       -      获取到的物理地址可以使用 VIRTUAL() 宏转换为内核段线性映射的虚拟地址
