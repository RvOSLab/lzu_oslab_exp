...
void mem_init()
{
    /** TODO: 设用户内存空间 [LOW_MEM, HIGH_MEM) 为可用 */
    /** TODO: 设SBI与内核内存空间 [MEM_START, LOW_MEM) 的内存空间为不可用 */
...
{
    /** TODO: 从用户内存空间高地址往低地址查找可用页 */
    /** TODO: 找到则分配内存页、使用 memset() 函数清空对应内存并返回内存地址 */
    /** TODO: 未找到则返回 0 */
}
...
uint64_t put_page(uint64_t page, uint64_t addr, uint16_t flag)
{
    ...
    for (size_t level = 0; level < 2; ++level) {
        // TODO: [lab4] 完成循环体，实现以下功能:
        //       - 在查找过程中分配各级未分配的页表
        //       - 完成查找功能(page_table)
    }
    // TODO: [lab4] 正确设置页表项 page_table[vpns[2]]
    return page;
}
...
{
    // 调用路径: [中断上下文] -> __alltraps -> trap -> trap_dispatch
    //           -> exception_handler -> page_fault_handler
    //           -> is_valid_but_fault -> un_wp_page
    uint64_t old_page, new_page;
    old_page = GET_PAGE_ADDR(*table_entry);
    
    // TODO: [lab4] 完成函数，处理以下写保护异常情况:
    //       - 试图 写 写时复制页 时         触发 写保护异常 (old_page >= LOW_MEM && mem_map[MAP_NR(old_page)] > 1)
    //       - 试图 写 写时复制页 时 最后一次 触发 写保护异常 (old_page >= LOW_MEM && mem_map[MAP_NR(old_page)] == 1)

    //       应完成以下要求:
    //       - 依照写时复制原理进行实现
    //       - 更新维护相关虚拟页引用计数
    //       - 在更新页表项后刷新页表缓存
    //       - 可使用 copy_page() 函数完成页内容的复制
    //       - 可使用 GET_FLAG() 宏获取 PTE 中所有的标志位
    //       - 可使用 VIRTUAL() 宏获取内核段的物理地址到虚拟地址的线性偏移
}
