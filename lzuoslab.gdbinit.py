...
class EnumDataStructure(gdb.Command):
    def __init__(self):
        super(EnumDataStructure, self).__init__ ("type-wrap", gdb.COMMAND_USER)
    ...
    def invoke(self, arg, from_tty):
        ...
        if obj.type == List:
            ...
            if len(argv) == 1:
                linked_list_map.pop(int(obj.address))
                return
            ...
            if len(argv) == 1:
                hash_table_map.pop(int(obj.address))
                return
            container_of, type_name = type_decorator(argv[1], argv[2])
